port module Textures exposing (Msg, Model, getTexture, getTextureFloat, init, LoadedTexture, TextureToLoad(..), TexturesState(..), getState, update, subscription)
...
init texturesToLoad =
    let
        ...
        texturesToLoadCount = List.length standardTexturesLoadingTasks + List.length generateTexturesCommands
    in
            ...
            [ standardTexturesLoadingTasks
                 ...
                 |> Task.attempt
                    (\result -> case result of
                        Ok loadedTextures -> StandardTexturesLoaded loadedTextures
                        Err _ -> TextureLoadingError
                    )
...
update msg (Textures state prevTextures) =
    case (msg, state) of
        (StandardTexturesLoaded textures, RemainingTextures remaining) ->
            let
                newRemaining = remaining - Dict.size textures
                newTextures = Dict.union prevTextures textures
            in
        ...
        (SignTextureLoaded fileName texture, RemainingTextures remaining) ->
            let
                ...
                newTextures = Dict.insert fileName texture prevTextures
            in
                if newRemaining <= 0 then
                    (Textures TexturesLoaded newTextures, Cmd.none)
                else
